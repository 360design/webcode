...
//            add_action('atmf_hidden_data_show', array($this , 'hidden_data_show') );
  ...
  wp_enqueue_script('jquery');
	       ...
	       //    wp_register_script('jquery2',  '//code.jquery.com/jquery-1.10.2.js', array(), false);
             //  wp_enqueue_script('jquery2');

			  // wp_register_script('jquery-ui',  '//code.jquery.com/ui/1.11.4/jquery-ui.js', array('jquery2'), false);
            //   wp_enqueue_script('jquery-ui');
				...
				wp_register_script('angular-chosen-jquery', UOU_ATMF_URL . '/assets/js/vendor/chosen/chosen.jquery.js', array(), false, true);
                wp_enqueue_script('angular-chosen-jquery');

				#wp_register_script('angular-chosen', UOU_ATMF_URL . '/assets/js/vendor/chosen/chosen.js', array(), false, true);
                #wp_enqueue_script('angular-chosen');
        ...
        public function atmf_admin_load_scripts($hook){
			...
			wp_register_style( 'chosen-css', UOU_ATMF_URL. '/assets/js/vendor/chosen/chosen.css', array(), false, 'all' );
            wp_enqueue_style( 'chosen-css' );

			wp_register_style( 'chosen-spinner-css', UOU_ATMF_URL. '/assets/css/chosen-spinner.css', array(), false, 'all' );
            wp_enqueue_style( 'chosen-spinner-css' );
            ...
            foreach ( $metadata as $item) {
                ...
                if( isset($item->option)  && $item->option == 'taxonomy' ){
                    ...
                    if( isset($item->metakey) ){
						...
						if($item->title == "Ansamblu rezidential")
						{

						$get_meta_value = $this->get_meta_values2( $item->metakey , $post_type );
		...
		public function get_meta_values2( $key = '', $type = 'post', $status = 'publish' ) {
			...
			foreach($r as $r2)
			{
				$arr[$r2] = $wpdb->get_var( $wpdb->prepare( "
                      SELECT post_title FROM {$wpdb->posts} WHERE ID = '%s'
